...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Traduction logic — menu déroulant EN / FR / NL
  const langSelect = document.getElementById('langSelect');
  if (langSelect) {
    langSelect.addEventListener('change', (e) => {
      body.setAttribute('data-lang', e.target.value);
      // Prévient les autres panneaux (Parcours, Compétences...) qu'il faut
      // reconstruire leur contenu dans la nouvelle langue.
      document.dispatchEvent(new CustomEvent('app:langchange', { detail: { lang: e.target.value } }));
    });
  }
