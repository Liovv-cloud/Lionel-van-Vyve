...
const SKILLS_CONTACT_INFO = {
  ...
  LINKEDIN: "https://www.linkedin.com/in/lionel-van-vyve-40168a161/",
  INSTAGRAM: "https://www.instagram.com/map_ane/"
};
...
function initSkillsPanel() {
  ...
  function showTooltip(app) {
    ...
    if (contactValue) {
      ...
      const contactKey = app.name.toUpperCase();
      if (contactKey === 'GMAIL') {
        link.href = `mailto:${contactValue}`;
      } else if (contactKey === 'TELEPHONE') {
        link.href = `tel:${contactValue.replace(/\s+/g, '')}`;
      } else {
        // Liens web (Instagram, LinkedIn, ...) : ouvrir la vraie page dans
        // un nouvel onglet au lieu de tenter un "tel:" invalide.
        link.href = contactValue;
        link.target = '_blank';
  ...
  function openPanel() {
    ...
    document.documentElement.classList.add('panel-open', 'skills-open');
    document.body.classList.add('panel-open', 'skills-open');
    activeCategory = null;
    folderView.classList.remove('open');
    folderView.classList.add('hidden');
  ...
  function closePanel() {
    ...
    document.documentElement.classList.remove('panel-open', 'skills-open');
    document.body.classList.remove('panel-open', 'skills-open');
    hideTooltip();
    folderView.classList.remove('open');
    folderView.classList.add('hidden');
    activeCategory = null;
  ...
  // Rebuild on language change (menu déroulant EN / FR / NL)
